...
`Input.setVirtualPosition()` 让真实指针、键盘自动圆周和幽灵手指共享同一个 NDC 坐标入口，所以引导必然推动真实目标球与蛇体。`ExperienceUI` 在有效半径内累计同向极角，达到 `1.65π` 进入 1.8 秒完成态；松手保留 2.4 秒，随后用 900 毫秒回落。完成时目标回到屏幕中心，fragment shader 的 `u_ritual` 窄带从尾扫向头，结束后恢复 `-1`。
